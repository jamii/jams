...
// The ixes here are initially null and get set by arrange.
...
pub fn planRelation(self: *Self, node_id: anytype) Error!RelationExprId {
    ...
    switch (@TypeOf(node)) {
        ...
        N.row => {
            ...
            for (node.exprs.get(p).expr.get(p).elements) |expr, expr_ix| {
